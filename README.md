...
Note: If any subprocess is created by calling `ShellExecute` (`ShellExecuteEx`) with the `open` verb, the timing is also correct because `ShellExecute` is a high-level API and the subprocess is created by an internal call to `CreateProcess`. However, if the subprocess is created by calling `ShellExecute` with the `runas` verb, the time consumed by the subprocess will not be recorded because the user security context is changed. It is also reasonable to not record the time consumed by the subprocess in other user security contexts.
